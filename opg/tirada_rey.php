...
if ($tirada_real == 'true') {
    ...
    if ($has_full_haki && ($has_clan_d || $has_dragon)) {
        $tirada_random = rand(1, 9);
        $rama_calculo = 'FULL_HAKI_Y_CLAN_D_O_DRAGON';
        $rango_tirada = '1-9';
    } else if ($has_full_haki) {
        $tirada_random = rand(1, 10);
        $rama_calculo = 'FULL_HAKI';
        $rango_tirada = '1-10';
    } else if ($has_clan_d || $has_dragon) {
        $tirada_random = rand(1, 19);
        $rama_calculo = 'CLAN_D_O_DRAGON';
        $rango_tirada = '1-19';
    } else {
        $tirada_random = rand(1, 20);
        $rama_calculo = 'NORMAL';
    ...
    $bonificaciones = sprintf(
        "FullHaki:%s | ClanD:%s | Dragon:%s",
        $has_full_haki ? 'SI' : 'NO',
        $has_clan_d ? 'SI' : 'NO',
        $has_dragon ? 'SI' : 'NO'
    );
    ...
    $log_entry = sprintf(
        "[%s] UID: %s | Username: %s | RAMA: %s | Rango: %s | Tirada: %d | Bonif: %s | Timestamp: %s | Microtime: %.6f | IP: %s | UserAgent: %s | Method: %s | POST: %s | Server: %s\n",
