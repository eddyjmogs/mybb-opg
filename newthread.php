...
$day_input = $mybb->get_input('day_input');
$estacion_input = $mybb->get_input('estacion_input');
$year_input = $mybb->get_input('year_input');
...
$query_mundo_rol = $db->query(" SELECT * FROM `mybb_forums` WHERE parentlist LIKE '%,$fid' AND parentlist LIKE '10,%'; ");
while ($q = $db->fetch_array($query_mundo_rol)) { $is_mundo_rol = true; }
...
if($mybb->input['action'] == "do_newthread" && $mybb->request_method == "post")
{
	...
	else
	{
		...
		$db->query(" 
			UPDATE `mybb_threads` SET `year`='$year_input',`estacion`='$estacion_input',`day`='$day_input' WHERE tid='$tid'
		");
