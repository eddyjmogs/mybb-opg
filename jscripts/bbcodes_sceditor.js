$(function ($) {
	...
	$.sceditor.formats.bbcode.set('font', {
		format: function (element, content) {
			...
			var editor_body = $('body', iframe.contents());
                        ...
                        // ### Own code
			if (font == "'Open Sans', Arial, Tahoma, sans-serif") { font = ''; }
                        if (font == '"Open Sans", Arial, Tahoma, sans-serif') { font = ''; }
	...
	/* Codigos del Foro de Rol*/

	// CODE PERSONAJE
	$.sceditor.command.set('personaje', {
		_dropDown: function (editor, caller) {
			var $content;
			
			$content = $(`
				<div>
					<div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Personaje</b></div>
					<div>
						<label for="mod">Tid del tema anterior (Opcional):</label>
			...
			$content.find('.button').on('click', function (e) {
				var mod = $content.find('#mod').val();
				var attrs = "";

				if(mod !== undefined && mod!=="")
					attrs="="+mod;

				editor.insert('[personaje' + attrs + ']');

				editor.closeDropDown(true);
				e.preventDefault();
			});

			editor.createDropDown(caller, 'insertpersonaje', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('personaje')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('personaje')._dropDown(this, caller);
		},
		tooltip: 'Code Personaje'
	});

	// CODE TECNICA
	$.sceditor.command.set('tecnica', {
		_dropDown: function (editor, caller) {
			var $content;

			$content = $(
				'<div> <div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Técnica</b></div>' +
				'<div>' +
				'<label for="mod">ID de la técnica:</label> ' +
			...
			editor.createDropDown(caller, 'inserttecnica', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('tecnica')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('tecnica')._dropDown(this, caller);
		},
		tooltip: 'Code Técnica'
	});

	// CODE ARMA
	$.sceditor.command.set('arma', {
		_dropDown: function (editor, caller) {
			var $content;

			$content = $(
				'<div> <div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Arma</b></div>' +
				'<div>' +
				'<label for="mod">ID del <a href="/sg/objetos.php" target="_blank" style=" color: #07607c; text-decoration: underline; ">arma</a>:</label> ' +
				'<input type="text" id="mod" />' +
				'</div>' +
				'<div>' +
				'<input type="button" class="button" value="' + editor._('Insert') + '" />' +
			...
			editor.createDropDown(caller, 'insertarma', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('arma')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('arma')._dropDown(this, caller);
		},
	...
	// CODE VIDA
	$.sceditor.command.set('vida', {
		_dropDown: function (editor, caller) {
			var $content;

			$content = $(
				'<div> <div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Vida</b></div>' +
				'<div>' +
				'<label for="mod1">Vida actual:</label> ' +
				'<input type="text" id="mod1" />' +
				'</div>' +
				'<div>' +
				'<label for="mod2">Vida máxima:</label> ' +
			...
			$content.find('.button').on('click', function (e) {
				var mod1 = $content.find('#mod1').val();
				var mod2 = $content.find('#mod2').val();
				var attrs = "";

				if (mod1 !== undefined && mod1 !== "" && mod2 !== undefined && mod2 !== "") {
					editor.insert(`[vida=${mod1},${mod2}]`);
				}
			...
			editor.createDropDown(caller, 'insertvida', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('vida')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('vida')._dropDown(this, caller);
		},
	...
	// CODE ENERGIA
	$.sceditor.command.set('energia', {
		_dropDown: function (editor, caller) {
			var $content;

			$content = $(
				'<div> <div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Energía</b></div>' +
				'<div>' +
				'<label for="mod1">Energía actual:</label> ' +
				'<input type="text" id="mod1" />' +
				'</div>' +
				'<div>' +
				'<label for="mod2">Energía máxima:</label> ' +
				'<input type="text" id="mod2" />' +
				'</div>' +
				'<div>' +
				'<input type="button" class="button" value="' + editor._('Insert') + '" />' +
			...
			$content.find('.button').on('click', function (e) {
				...
				if (mod1 !== undefined && mod1 !== "" && mod2 !== undefined && mod2 !== "") {
					editor.insert(`[energia=${mod1},${mod2}]`);
				}
			...
			editor.createDropDown(caller, 'insertenergia', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('energia')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('energia')._dropDown(this, caller);
		},
		tooltip: 'Code Energía'
	});

	// CODE HAKI
	$.sceditor.command.set('haki', {
		_dropDown: function (editor, caller) {
			var $content;

			$content = $(
				'<div> <div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Haki</b></div>' +
				'<div>' +
				'<label for="mod1">Haki actual:</label> ' +
				'<input type="text" id="mod1" />' +
				'</div>' +
				'<div>' +
				'<label for="mod2">Haki máxima:</label> ' +
				'<input type="text" id="mod2" />' +
				'</div>' +
				'<div>' +
				'<input type="button" class="button" value="' + editor._('Insert') + '" />' +
			...
			editor.createDropDown(caller, 'inserthaki', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('haki')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('haki')._dropDown(this, caller);
		},
	...
	// CODE DADO
	$.sceditor.command.set('dado', {
		_dropDown: function (editor, caller) {
			var $content;

			$content = $(
				'<div> <div style="text-align: center; padding: 0px 0 10px 0;"><b>Código de Dados</b></div>' +
				'<div>' +
				'<label for="mod1">¿Cuántos dados?:</label> ' +
				'<input type="number" id="mod1" value="1" />' +
				'</div>' +
				'<div>' +
				'<label for="mod2">¿Cuántas caras?:</label> ' +
				'<input type="number" id="mod2" value="6" />' +
				'</div>' +
				'<div>' +
				'<input type="button" class="button" value="' + editor._('Insert') + '" />' +
				'</div>' +
			...
			$content.find('.button').on('click', function (e) {
				...
				if (mod1 !== undefined && mod1 !== "" && mod2 !== undefined && mod2 !== "") {
					editor.insert(`[dado=${mod1}d${mod2}]`);
				}
			...
			editor.createDropDown(caller, 'insertdado', $content.get(0));
		},
		exec: function (caller) {
			$.sceditor.command.get('dado')._dropDown(this, caller);
		},
		txtExec: function (caller) {
			$.sceditor.command.get('dado')._dropDown(this, caller);
		},
	...
	$.sceditor.command.set("spoiler", {
		_dropDown: function (editor, caller) {
			...
			$content = $(
				'<div>' +
				'<div>' +
				'<label for="spoiler">Código de ' + editor._('Spoiler') + ':</label> ' +
				'<textarea type="text" id="spoiler"></textarea>' +
				'</div>' +
				'<div><input type="button" class="button" value="' + editor._('Insert') + '" /></div>' +
				'</div>'
			);
			...
			setTimeout(function () {
				$content.find('#spoiler').trigger('focus');
			}, 100);

			$content.find('.button').on('click', function (e) {
				var val = $content.find('#spoiler').val(),
					before = '[spoiler]',
					end = '[/spoiler]';
	...
	$.sceditor.command.set("hide", {
		_dropDown: function (editor, caller) {
			...
			$content = $(
				'<div>' +
				'<div>' +
				'<label for="hide">' + editor._('Hide') + ':</label> ' +
			...
			setTimeout(function () {
				$content.find('#hide').trigger('focus');
			}, 100);

			$content.find('.button').on('click', function (e) {
				var val = $content.find('#hide').val(),
					before = '[hide]',
					end = '[/hide]';
			...
			editor.createDropDown(caller, 'inserthide', $content.get(0));
		},
		exec: function (caller) {
			if ($.trim(this.getRangeHelper().selectedRange())) {
			...
			$.sceditor.command.get('hide')._dropDown(this, caller);
		},
		txtExec: ['[hide]', '[/hide]'],
		tooltip: 'Hide'
	});
	...
	// CODE BELICO
	$.sceditor.command.set('belico', {
		exec: function () {
			this.insert('[belico]', '[/belico]');
		},
		txtExec: ['[belico]', '[/belico]'],
		tooltip: 'Código Bélico'
	});
