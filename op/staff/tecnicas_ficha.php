...
if ($accion && $tecnicas && $staff && $razon && $ficha_id && (is_mod($uid) || is_staff($uid))) {
    ...
    foreach ($tecnicas_array as $tec) {
        ...
        if ($clean_tec != "") {
            ...
            if ($accion == 'Añadir') {
                $db->query(" 
                    INSERT INTO `mybb_op_tec_aprendidas` (`tid`, `uid`) VALUES 
                    ('$clean_tec', '$ficha_id');
                ");
                $log_short = "Cambios de técnicas para ficha de UID: $ficha_id (" . $f_var['nombre'] . "):\n" . "-- $accion técnica ID: $tec\n";
                ...
                $db->query(" 
                    DELETE FROM `mybb_op_tec_aprendidas` WHERE tid='$clean_tec' AND uid='$ficha_id'; 
                ");
                $log .= "-- $accion técnica ID: $tec\n";
                $log_short = "Cambios de técnicas para ficha de UID: $ficha_id (" . $f_var['nombre'] . "):\n" . "-- $accion técnica ID: $tec\n";

                $db->query(" 
                    INSERT INTO `mybb_op_audit_consola_tec` (`staff`, `razon`, `log`) VALUES 
                    ('$staff', '$razon', '$log_short'); 
                "); 
