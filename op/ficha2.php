...
if ($ficha_existe == true && !$ficha_staff && ((is_mod($user_uid) || is_staff($user_uid)) || ($mybb->user['uid'] == $user_uid))) {
    ...
    if ($faccion == 'Pirata' && $reputacion < 51 && intval($nivel) < 5) {
        $rango = 'Pirata';
    } else if ($faccion == 'Pirata' && $rango = 'Pirata' && $reputacion >= 51 && intval($nivel) >= 5) {
        $rango = 'Corsario';
    } else if ($faccion == 'Pirata' && $rango = 'Corsario' && $reputacion >= 101 && intval($nivel) >= 10) {
        $rango = 'Bucanero';
    } else if ($faccion == 'Pirata' && $rango = 'Bucanero' && $reputacion >= 101 && intval($nivel) >= 15) {
        $rango = 'Lobo de Mar';
    }
    ...
    if ($faccion == 'Marina' && &reputacion < 26 && intval($nivel) < 3) {
        $rango = 'ReclutaM';
    } else if ($faccion == 'Marina' && $rango == 'ReclutaM' && $reputacion >= 26 && intval($nivel) >= 3) {
        $rango = 'SoldadoRaso';
    } else if ($faccion == 'Marina' && $rango == 'SoldadoRaso' && $reputacion >= 51 && intval($nivel) >= 7) {
        $rango = 'SargentoM';
    } else if ($faccion == 'Marina' && $rango == 'SargentoM' && $reputacion >= 51 && intval($nivel) >= 12) {
        $rango = 'Suboficial';
    }
    ...
    // eval("\$op_ficha_script3 = \"".$templates->get("op_ficha_script3")."\";");
    // eval("\$op_ficha_script4 = \"".$templates->get("op_ficha_script4")."\";");
    // eval("\$op_ficha_crear_script = \"".$templates->get("op_ficha_crear_script")."\";");
    eval("\$op_ficha_script = \"".$templates->get("op_ficha_script")."\";");
    eval("\$page = \"".$templates->get("op_ficha")."\";");
    output_page($page);
