...
function recentthread_list_threads($return=false, $threadcount=0, $page=1)
{
    ...
    // Initialize to empty strings — these are conditionally set below and used in SQL
    ...
    // Initialize variables that are conditionally assigned inside the loop
    $posteravatar = '';
    $lastavatar   = '';
    $modcol       = '';
    $modheader    = '';
    ...
    while($thread = $db->fetch_array($query))
    {
        ...
        if(is_moderator($thread['fid'], "caneditposts") || (!empty($fpermissions['caneditposts']) && $fpermissions['caneditposts'] == 1))
        {
            $can_edit_titles = 1;
        }
        else
