...
/**
 * Variante de build_profile_link() que apunta a la ficha de personaje
 * (/op/ficha.php?uid=X) en lugar del perfil estándar de MyBB.
 *
 * @param string $username Display name del usuario
 * @param int    $uid      ID de usuario (0 = invitado)
 * @param string $target   Atributo target del enlace (opcional)
 * @param string $onclick  Handler onclick JS (opcional)
 * @return string Etiqueta <a> HTML o nombre plano para invitados
 */
function build_profile_link_ficha($username="", $uid=0, $target="", $onclick="")
{
	global $mybb, $lang;
	...
	{
		$target_attr  = !empty($target)  ? " target=\"".htmlspecialchars_uni($target)."\"" : "";
		$onclick_attr = !empty($onclick) ? " onclick=\"".htmlspecialchars_uni($onclick)."\"" : "";
		$ficha_url    = $mybb->settings['bburl'] . "/op/ficha.php?uid=" . (int)$uid;

		return "<a href=\"{$ficha_url}\"{$target_attr}{$onclick_attr}>{$username}</a>";
	}
}
